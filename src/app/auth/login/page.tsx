...
import { checkUserVerification } from "./actions";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userRole: "Resident" | "Manager" =
        result.data.user.role === "WARGA" ? "Resident" : "Manager";
      type UserData = {
        id: string;
        email: string;
      ...
      const userData: UserData = {
        ...
        role: userRole,
        name: result.data.user.name || result.data.user.email, // Use name if available, fallback to email
      };
      login(userData.role, userData.name);

      if (userData.role === "Manager" || result.data.user.role === "ADMIN") {
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-slate-950">
      {/* ── LEFT SIDE – Form ── */}
      <div className="w-full lg:w-[45%] flex flex-col justify-center px-10 py-16 bg-white dark:bg-slate-900 relative">
        {/* Dark mode toggle */}
        <button
          suppressHydrationWarning
          onClick={toggleTheme}
          className="absolute top-6 right-6 p-2 rounded-full text-gray-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
        >
          {theme === "dark" ? (
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 hover:text-green-600 dark:hover:text-green-400 transition-colors"
              >
                {showPassword ? (
